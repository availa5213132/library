...
func SendSMS(accessKeyId, accessKeySecret, signature, templateCode, phoneNumber, code string) error {
	...
	//阿里云提供了多个服务（例如短信服务、对象存储服务、云服务器等），开发者可以使用相应的 SDK 来访问和管理这些服务。在使用 SDK 之前，需要创建一个客户端对象，
	//该客户端对象将用于与阿里云服务进行通信。
	//NewClientWithAccessKey 函数的作用是创建一个使用访问密钥进行身份验证的客户端对象。访问密钥由阿里云控制台提供，包括 AccessKeyId 和 AccessKeySecret。
	//这些密钥用于对 API 请求进行签名和身份验证，以确保请求的安全性和合法性。
	...
	key := GenerateVerificationCodeKey(phoneNumber)
	err = SetRedis(key, code) //存储在redis
	if err != nil {
		return err
	}
	return nil
...
func VerifyPhoneCode(phone, key, code string) bool {
	// 1. 从Redis获取存储的验证码
